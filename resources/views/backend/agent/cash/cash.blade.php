            ...
            <div class="box box-danger">
                ...
                <div class="box-footer">
                    ...
                    @if (Request::get('from_date') || Request::get('to_date'))
                        (@lang('app.results_date'):<span
                            class="text-blue">{{ date('Y-m-d h:i A', strtotime(Request::get('from_date'))) . ' to ' . date('Y-m-d h:i A', strtotime(Request::get('to_date'))) }}</span>
                        )
                    @endif
                </div>
            </div>
                ...
                <div class="pull-right box-tools">
                    @permission('users.add')
                        <a href="{{ route('backend.user.create') }}"
                            class="btn btn-block btn-primary btn-sm">@lang('app.add_user')</a>
                    @endpermission
                </div>
            </div>
            <div class="box-body">
                        ...
                        @forelse($result as $key => $value)
                            @if (
                                $value->total_bet != 0 ||
                                    $value->total_win != 0 ||
                                    $value->pl != 0 ||
                                    (!Request::get('username') && !Request::get('from_date') && !Request::get('to_date')))
                                <tr>
                                    <td><a href="{{ route('backend.user.edit', $value->id) }}">{{ $value->username }}</a>
                                    </td>
                                    <td>{{ $value->name }}</td>
                                    <td>{{ number_format(floatval($value->balance), 2, '.', '') }}</td>
                                    ...
                                    @if (auth()->user()->role_id != 10)
                                        <td class="d-flex justify-content-space padd-3">
                                            <a href="" class="btn btn-block btn-warning btn-sm ms-2"
                                                data-target="#set_score{{ $value->id }}" data-toggle="modal">
                                                @lang('app.set_score')
                                            </a>

                                            <a href="{{ route('backend.agent.log_filter', $value->id) }}"
                                                class="btn btn-block btn-warning btn-sm mt-0 ms-2">
                                                @lang('app.score_log')
                                            </a>

                                            <a href="{{ route('backend.user.edit', $value->id) }}"
                                                class="btn btn-block btn-warning btn-sm ms-2 mt-0 ">
                                                @lang('app.edit_a_p')
                                            </a>

                                            <div class="modal fade" id="set_score{{ $value->id }}" tabindex="-1"
                                                aria-hidden="true">
                                                    ...
                                                    <div class="modal-content">
                                                        <form action="{{ route('backend.agent.balance') }}" method="POST">
                                                            ...
                                                            <div class="modal-body">
                                                                <div class="form-group">
                                                                    <label for="balance{{ $value->id }}">
                                                                        @lang('app.current_score') :
                                                                ...
                                                                <div class="form-group">
                                                                    ...
                                                                    @if (auth()->user()->role_id == 3)
                                                                        <input type="number" step="0.01"
                                                                            class="form-control"
                                                                            id="balance{{ $value->id }}" name="balance"
                                                                            placeholder="@lang('app.sum')" required>
                                                                    @else
                                                                        <input type="number" max="{{ max_score() }}"
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <button type="submit"
                                                                    class="btn btn-primary">@lang('app.pay_in')</button>
                                                            </div>
                                                        </form>
                                                    </div>
                                                </div>
