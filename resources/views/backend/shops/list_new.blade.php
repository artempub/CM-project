...
@section('scripts')
    <script>
        ...
        var user_table = $('#user-datatable').DataTable({
            ...
            ajax: {
                url: "{{ route('backend.shop.listpost') }}",
                type: "POST",
                data: function(d){
            ...
            columnDefs: [{
                ...
                targets: 1,
                render: function(data, type, row) {
                    return '<a class="text-success text-center" href="shop/profile/' + row.id + '">' + row.username + "</a>"
                }
            }],
        ...
        $('#btn_change_operator').on('click', function(e){
            ...
            if (arr.length == 0) {
                ...
                myData ={};
                myData.sel_rows = arr
                myData.sel_new_parent_operator = $('#sel_new_parent_perator').val()
