...
namespace VanguardLTE {
    class User extends \Illuminate\Foundation\Auth\User implements \Tymon\JWTAuth\Contracts\JWTSubject
    {
        ...
        {
            $role = \jeremykenedy\LaravelRoles\Models\Role::where('id', $this->hasRole('agent') ? $this->role_id : $this->role_id - 1)->first();
            $userIds = $this->availableUsersByRole($role->slug);
            if (count($userIds)) {
                return User::whereIn('id', $userIds)->where('parent_id', $this->id)->get();
            }
            return false;
        }
        ...
        public function addBalance($type, $summ, $payeer = false, $refund = true, $system = 'handpay', $update_level = true, $model = false)
        {
            ...
            if (!$payeer->hasRole('admin')) {
                if ($payeer->hasRole('distributor')) {
                    if (!str_contains($this->ancestry, $payeer->ancestry)) {
                        return json_encode([
                            'status' => 'error',
                            'message' => trans('app.wrong_user')
                        ]);
                    }
                } else if ($payeer->id != $this->parent_id) {
                    return json_encode([
                        'status' => 'error',
                        'message' => trans('app.wrong_user')
                    ]);
                ...
                if ($payeer->hasRole('distributor') && !$this->hasRole(['agent', 'distributor'])) {
                    return json_encode([
                        'status' => 'error',
                        'message' => trans('app.wrong_user')
                    ]);
            ...
            if (!$payeer->hasRole('admin')) {
                if ($type == 'add' && $payeer->balance < $summ) {
                    return json_encode([
                        'status' => 'error',
                        'message' => trans('app.not_enough_money_in_the_agent_balance', [
            ...
            if ($payeer->hasRole('agent') && $this->hasRole('user')) {
            ...
            if (!isset($_obf_0D2A375C16264003061027131E1C3728380B0521071E01[$system]) && $payeer->hasRole('agent')) {
            ...
            $balance = $summ;
            if ($payeer->hasRole('agent') && $this->hasRole('user') && $type == 'add' && $happyhour && $system == 'handpay') {
                ...
                if (isset($_obf_0D2A375C16264003061027131E1C3728380B0521071E01[$system])) {
                    if ($model) {
                        ...
                        $this->increment('count_' . $_obf_0D2A375C16264003061027131E1C3728380B0521071E01[$system], $summ * ($model->wager ?: 1));
                        $this->increment('address', $summ);
                    } else if ($system == 'cashback') {
                        $this->increment($_obf_0D2A375C16264003061027131E1C3728380B0521071E01[$system], $summ);
                        $this->increment('count_' . $_obf_0D2A375C16264003061027131E1C3728380B0521071E01[$system], $summ);
                    }
                } else {
            ...
            if (!$payeer->hasRole('admin') && ($system != "cashback" && $system != "welcome_bonus")) {
                $payeer->update(['balance' => $payeer->balance - $summ]);
            }
            if ($payeer->hasRole('agent') && $this->hasRole('user') && !isset($_obf_0D2A375C16264003061027131E1C3728380B0521071E01[$system])) {
                /*if( $type == 'out' )
                {
            ...
            if ($this->hasRole('user')) {
                // $stattt = Lib\CashbackLib::action($this->id);
                Lib\WBLib::action($this->id);
                if ($system == 'handpay' || $system == 'btcpayserver' || $system == 'coinbase' || $system == 'interkassa') {
                    Lib\CBLib::action($this->id, $summ);
                };
            }
            return json_encode([
                'status' => 'success',
                // 'message' => $stattt?:trans('app.balance_updated')
        ...
        public function addLimit($type, $summ, $payeer = false,  $system = 'handpay', $update_level = true, $model = false)
        {
            ...
            if ($payeer->hasRole('admin') && !$this->hasRole('distributor')) {
                return json_encode([
                    'status' => 'error',
                    'message' => trans('app.wrong_user')
                ]);
            }
            if ($payeer->hasRole('distributor') && !$this->hasRole('agent')) {
                return json_encode([
                    'status' => 'error',
                    'message' => trans('app.wrong_user')
                ]);
            ...
            if ($payeer->hasRole('admin') && $this->hasRole('distributor') || $payeer->hasRole('distributor') && $this->hasRole('agent')) {
            ...
            if (count($distributor->shops()) && ($shops = $distributor->rel_shops)) {
                foreach ($shops as $shop) {
                    if ($shop = $shop->shop) {
                        ...
                        if (($agents = $distributor->getInnerUsers())) {
                            foreach ($agents as $agent) {
                                $return['shops'][$shop->id]['agents'][$agent->id] = [
                                    'href' => route('backend.user.edit', ['user' => $agent->id], false),
                                    'text' => ($agent->username ?: trans('app.n_a')),
                                    'balance' => User::where([
                                        'role_id' => 1,
