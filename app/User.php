...
namespace VanguardLTE {
    class User extends \Illuminate\Foundation\Auth\User implements \Tymon\JWTAuth\Contracts\JWTSubject
    {
        ...
        {
            //get the user of the ID which is same as parent_id of the shop_user
            return $this->hasOne('VanguardLTE\Operator', 'user_id'); //foreign key, local key
        }
        public function childs()
        {
            return $this->hasMany('VanguardLTE\User', 'parent_id', 'id');
        }
        public function parents()
        {
            //get the user of the ID which is same as parent_id of the shop_user
            return $this->hasMany('VanguardLTE\User', 'id', 'parent_id'); //foreign key, local key
        }
        //to get parent username of operator
        public function parent_operator()
        {
            //get the user of the ID which is same as parent_id of the shop_user
            return $this->hasOne('VanguardLTE\User', 'id', 'parent_id'); //foreign key, local key
        }
        ...
        public function child_shops()
        {
            return $this->hasMany('VanguardLTE\Transaction', 'id');
        }
        ...
        public function shops($onlyId = false)
        {
            ...
            if (count($shops)) {
                if ($onlyId) {
                    return Shop::whereIn('id', $shops)->orWhere('user_id', $this->id)->pluck('id');
                } else {
                    return Shop::whereIn('id', $shops)->orWhere('user_id', $this->id)->pluck('name', 'id');
                }
            } else {
                return [];
            }
        ...
        {
            return $this->role_id == 1 ? $this->belongsTo('VanguardLTE\Shop', 'shop_id') : $this->hasOne('VanguardLTE\Shop', 'user_id');
        ...
        public function addBalance($type, $summ, $payeer = false, $refund = true, $system = 'handpay', $update_level = true, $model = false)
        {
            ...
            if ($this->hasRole('manager') || $this->hasRole('shop')) {
                return json_encode([
                    'status' => 'error',
            ...
            if ($payeer->hasRole('shop') && !$this->hasRole('user')) {
                return json_encode([
                    'status' => 'error',
                    'message' => trans('app.wrong_user')
                ]);
            ...
            if (!isset($_obf_0D2A375C16264003061027131E1C3728380B0521071E01[$system]) && $payeer->hasRole('shop') && $this->hasRole('user')) {
            ...
            $balance = $summ;
            if ($payeer->hasRole('shop') && $this->hasRole('user') && $type == 'add' && $happyhour && $system == 'handpay') {
            ...
            if ($payeer->hasRole('shop') && $this->hasRole('user') && !isset($_obf_0D2A375C16264003061027131E1C3728380B0521071E01[$system])) {
