...
namespace VanguardLTE\Http\Controllers\Api
{
    ...
    class ShopController extends ApiController
    {
        ...
        public function admin(\Illuminate\Http\Request $request)
        {
            ...
            if( $usersBalance < 0 || $agentBalance < 0 || $distributorBalance < 0 || $shopBalance < 0 ) 
            {
                return redirect()->back()->withErrors(['Error balance < 0'])->withInput();
            }
            if( ($agentBalance > 0 || $shopBalance > 0) && ($distributorBalance <= 0 || $distributorBalance < ($agentBalance + $shopBalance) )) 
            {
                return redirect()->back()->withErrors(['Error balance: Agent + Shop > Distributor'])->withInput();
            }
            if( $usersBalance > 0 && ($agentBalance <= 0 || $agentBalance < $usersBalance) ) 
            {
                return redirect()->back()->withErrors(['Error balance: Users > Agent'])->withInput();
            }
            $roles = \jeremykenedy\LaravelRoles\Models\Role::get();
            $distributor = \VanguardLTE\User::create($distributor + [
                'parent_id' => auth()->user()->id, 
                'ancestry' => auth()->user()->ancestry.auth()->user()->id.'/', 
                'role_id' => 3, 
            ...
            {
                $distributor->addBalance('add', $distributorBalance);
                sleep(1);
            }
            if( $agentBalance > 0 ) 
            {
                $agent->addBalance('add', $agentBalance, $distributor);
                sleep(1);
            }
            if( $shopBalance > 0 ) 
            {
        ...
        public function destroy($shop)
        {
            ...
            if( !auth()->user()->hasRole('admin') ) 
            {
                ...
                $distributors = \VanguardLTE\User::where('role_id', 3)->whereHas('rel_shops', function($query) use ($shop)
                {
