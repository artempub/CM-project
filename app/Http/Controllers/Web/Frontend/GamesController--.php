...
namespace VanguardLTE\Http\Controllers\Web\Frontend {
    ...
    class GamesController extends \VanguardLTE\Http\Controllers\Controller
    {
        public function index(\Illuminate\Http\Request $request, $category1 = '', $category2 = '')
        {
            ...
            if (!\Illuminate\Support\Facades\Auth::check() && $frontend == "Tropicoblack") {
            ...
            if ($category1 != '') {
                $cat1 = \VanguardLTE\Category::where(['href' => $category1, 'parent' => 0])->first();
            ...
            if (settings('use_my_games') && $category1 == 'my_games' && \VanguardLTE\Lib\GetHotNewMyGames::get_my_games(false)) {
                $currentSliderNum = 'my_games';
            }
            if (settings('use_new_categories') && $category1 == 'new' && \VanguardLTE\Lib\GetHotNewMyGames::get_new_games(false)) {
                $currentSliderNum = 'new';
            }
            if (settings('use_hot_categories') && $category1 == 'hot' && \VanguardLTE\Lib\GetHotNewMyGames::get_hot_games(false)) {
            ...
            if ($request->input('game')) {
                if ($request->input('is_demo')) {
                    $link = route('frontend.game.go.prego', [$request->input('game'), '1']);
                } else {
                    $link = route('frontend.game.go', $request->input('game'));
                }
                return view('frontend.' . $frontend . '.games.list', compact('games', 'category1', 'cat1', 'categories', 'currentSliderNum', 'title', 'body', 'keywords', 'description', 'jpgs', 'shop', 'devices', 'tournament', 'is_game_page', 'jpgSum', 'gamestat', 'depositlist', 'withdrawlist', 'link'));
            }
        ...
        public function balanceAdd(\Illuminate\Http\Request $request)
        {
            ...
            if ($request->system == 'coinbase') {
                ...
                ])->post('http://api.commerce.coinbase.com/charges', [
        ...
        public function check_redirect(\Illuminate\Http\Request $request, $category1, $error = '')
        {
            ...
            if ($category1 == '') {
                ...
                if (settings('use_my_games') && \VanguardLTE\Lib\GetHotNewMyGames::get_my_games(false)) {
                    return $this->do_redirect($request, [
                        'category1' => 'my_games',
                        'page' => $request->cookie('currentPage')
                    ], $error);
                }
                if (settings('use_new_categories') && \VanguardLTE\Lib\GetHotNewMyGames::get_new_games(false)) {
                    return $this->do_redirect($request, [
                        'category1' => 'new',
                        'page' => $request->cookie('currentPage')
                    ], $error);
                }
                if (settings('use_hot_categories') && \VanguardLTE\Lib\GetHotNewMyGames::get_hot_games(false)) {
                    return $this->do_redirect($request, [
                        'category1' => 'hot',
                        'page' => $request->cookie('currentPage')
                    ], $error);
        ...
        public function search(\Illuminate\Http\Request $request)
        {
            ...
            $games = $games->get();
            $returns = '';
            $currency = (\Illuminate\Support\Facades\Auth::check() && auth()->user()->present()->shop ? auth()->user()->present()->shop->currency : '');
            if (count($games)) {
                foreach ($games as $game) {
                    $returns .= view('frontend.Default.partials.game', compact('game', 'currency', 'detect', 'category1'));
                }
            }
            return json_encode([
                'success' => false,
        ...
        public function bonuses()
        {
            ...
            return view('frontend.' . settings('frontend') . '.pages.bonuses', compact('currentSliderNum', 'category1', 'categories', 'bonuses', 'shop', 'jpgs'));
        }
