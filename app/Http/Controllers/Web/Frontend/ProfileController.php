...
namespace VanguardLTE\Http\Controllers\Web\Frontend
{
    ...
    class ProfileController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function withdraw(\VanguardLTE\Http\Requests\User\WithdrawRequest $request)
        {
            ...
            //---user can't get free cashback---//
            if($user->cashback){
                $bet = \VanguardLTE\StatGame::where('user_id', auth()->user()->id)->sum('bet');
                $win = \VanguardLTE\StatGame::where('user_id', auth()->user()->id)->sum('win');
                $cashback = $user->cashback;
                $lock = $cashback - $bet - $win;
    
                if($user->balance >= $txtamount){
                    $rest = $user->balance - $txtamount;
                    if($rest < $lock){
                        return redirect()->back()->withErrors('Please play game to cashback');
                    } else if($rest < $cashback){
                        $user->update(["cashback" => $rest]);
                    }
                }
            }
            //--------------------------------------//
