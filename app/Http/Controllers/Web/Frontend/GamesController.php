...
namespace VanguardLTE\Http\Controllers\Web\Frontend {
    ...
    class GamesController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function index(\Illuminate\Http\Request $request, $category1 = '', $category2 = '')
        {
            ...
            // if (false && $category1 == 'apollo' || $category1 == '') {
            //     $device = [0, 2];
            //     if ($this->agent->isMobile()) $device = [1, 2];

            //     $games = \VanguardLTE\ApolloGames::whereIn('device', $device)->where('flash', 0);

            //     if (isset($request->category)) {
            //         $games = $games->where('categories', $request->category);
            //     }
            //     if (isset($request->title)) {
            //         $games = $games->where('title', $request->title);
            //     }

            //     $games = $games->select('gameId', 'name', 'img', 'title')->get();
            //     $titles = \VanguardLTE\ApolloGames::distinct()->where('flash', 0)->pluck('title')->toArray();
            //     $categories = \VanguardLTE\ApolloGames::distinct()->where('flash', 0)->pluck('categories')->toArray();

            //     // dd($categories);
            //     $shop_id = (\Illuminate\Support\Facades\Auth::check() ? auth()->user()->shop_id : 0);
            //     $jpgs = \VanguardLTE\JPG::where('shop_id', $shop_id)->get();

            //     return view('frontend.Apollo.games.list', compact('games', 'jpgs', 'titles', 'categories'));
            // }
        ...
        public function server(\Illuminate\Http\Request $request, $game)
        {
            ...
            $server = new $object();
            \Illuminate\Support\Facades\Log::info('before echo');
            \VanguardLTE\Lib\CBLib::check(auth()->user()->id);
            echo $server->get($request, $game, auth()->user()->id);
            \Illuminate\Support\Facades\Log::info('echo echo echo');
            //  \VanguardLTE\Lib\CBLib::check(auth()->user()->id);
        }
