...
use \Illuminate\Http\Request; {
    ...
    class AuthController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function andreas_apiLogin($game)
        {
            $response = \Illuminate\Support\Facades\Http::withHeaders([
                'Accept' => 'application/json',
                'Content-Type' => 'application/json'
            ])->post('http://tbs2api.dark-a.com/API/openGame/', [
                'cmd' => 'openGame',
                'hall' => '3199760',
                'key' => '8ccb7ff06112022',
                "domain" => "http://46.166.161.103",
                "exitUrl" => "http://46.166.161.103",
                "language" => "en",
                "continent" => "eur",
                "login" => "" . auth()->user()->id . "",
                "gameId" => "" . $game . "",
            ...
            if ($responseList) {
                return redirect()->to($responseList['content']['game']['url']);
                // return response()->json([
                //     'status' => 'ok',
                //     'url' => $responseList['content']['game']['url']
                // ]);
            }
        }
        public function andreas_apiLoginPost()
        {
            $input = file_get_contents('php://input');
            $game_uuid = substr($input, 5);
        ...
        protected function handleUserWasAuthenticated(\Illuminate\Http\Request $request, $throttles, $user)
        {
            ...
            return redirect()->intended("/categories/amatic");
        }
