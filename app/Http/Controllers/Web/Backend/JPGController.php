...
namespace VanguardLTE\Http\Controllers\Web\Backend
{
    ...
    class JPGController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function index(\Illuminate\Http\Request $request)
        {
            ...
            $jackpots = \VanguardLTE\JPG::where('user_id', auth()->user()->id)->get();
            //check if I have permission to edit jackpot
            $edit_jpt_enabled = \VanguardLTE\Permissions_operator::where('user_id', auth()->user()->id)
            ...
            if(count($jackpots) == 0)
            {
                ...
                \DB::statement('insert '.\DB::getTablePrefix().$jpg_table.'(name,balance,start_balance,pay_sum,percent,shop_id) (select name,balance,start_balance,pay_sum,percent, '.auth()->user()->shop_id.' as shop_id from '.\DB::getTablePrefix().$jpg_table.' where shop_id=0)');
                $jackpots = \VanguardLTE\JPG::where('shop_id', auth()->user()->shop_id)->get();//Should consider Shop_id in the meantime...
            }
            if (auth()->user()->hasRole('shop') ) {
                # code...
                return view('backend.jpg.list_shop', compact('jackpots', 'edit_jpt_enabled'));
            }else{
                return view('backend.jpg.list', compact('jackpots', 'edit_jpt_enabled'));
