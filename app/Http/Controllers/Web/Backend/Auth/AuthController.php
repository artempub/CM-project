...
use Session;
{
    ...
    class AuthController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        {
            // var_dump('here is login for backend');die();
            $directories = [];
        ...
        {
            return view('backend.auth.cashierlogin');
        }
        ...
        public function postLoginCashier(\VanguardLTE\Http\Requests\Auth\LoginRequest $request, \VanguardLTE\Repositories\Session\SessionRepository $sessionRepository){
            $throttles = settings('throttle_enabled');
            if( $throttles && $this->hasTooManyLoginAttempts($request) ) 
            {
                return $this->sendLockoutResponse($request);
            ...
            if( !\Auth::validate($credentials) ) 
            {
                ...
                return redirect()->to('cashier/login')->withErrors(trans('auth.failed'));
            }
            $user = \Auth::getProvider()->retrieveByCredentials($credentials);
            if(!$user->hasRole('shop')){
                return redirect()->route('cashier.auth.login')->withErrors(trans('app.wrong_user'));
            }
            if( settings('siteisclosed') ) 
            {
                \Auth::logout();
                return redirect()->route('cashier.auth.login')->withErrors(trans('app.site_is_turned_off'));
            }
            if( $user->isBlocked()) 
            {
                return redirect()->to('cashier/login')->withErrors(trans('app.your_shop_is_blocked'));
            }
            if( $user->isBanned())
            {
                return redirect()->to('cashier/login')->withErrors(trans('app.your_account_is_banned'));
            }
            if( settings('reset_authentication') && $user->hasRole('user') && count($sessionRepository->getUserSessions(\Auth::id())) ) 
            {
                foreach( $sessionRepository->getUserSessions($user->id) as $session ) 
        ...
        protected function handleUserWasAuthenticated(\Illuminate\Http\Request $request, $throttles, $user)
        {
            ...
            if( !\Auth::user()->hasPermission('dashboard') || \Auth::user()->hasRole('shop') ) 
            {
