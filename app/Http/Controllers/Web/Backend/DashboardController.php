...

{
    ...
    class DashboardController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        {
            // if (null !== (\Auth::user()->hasRole('cashier')))
            // {
            //     // var_dump('cashier');die;
            ...
            if (!auth()->user()->hasPermission('dashboard'))
            {
                ...
                //     return redirect()->route('backend.user.list');
                // }else{
                //     return redirect()->route('backend.shop.home');
        ...
        //use for Operator role's Home page in new user hierarchy, by 2worldsoft
        public function index_new(\Illuminate\Http\Request $request)
        {
            ...
            $this->shop_roleId =  \jeremykenedy\LaravelRoles\Models\Role::where('slug', 'shop')->first()->id;
            $this->operator_roleId =  \jeremykenedy\LaravelRoles\Models\Role::where('slug', 'operator')->first()->id;
            //get Parent ID of you to collect all childs of the upper parent
            $parent_id = \VanguardLTE\User::find(auth()->user()->id)->parent_id;
            //Right now, replace it with your ID to avoid cofusion related with numbers of Upper parents
            $parent_id = auth()->user()->id;
            //get role from Frontend
            $sel_role_id = 0;
            if ($request->sel_role && $request->sel_role != 'all') {
                $sel_role_id = \jeremykenedy\LaravelRoles\Models\Role::where('slug', $request->sel_role)->first()->id;
            }
            $child_users = \VanguardLTE\User::find($parent_id)->childs;
            $this->get_hierarchy_childs_ids($child_users, $sel_role_id);
            if ($request->ajax()) {

                //With use of 'with'
                // $list = \VanguardLTE\Transaction::with('shop_user')->with('shop_to_user')->get();
                // With use of joins
                $users_from_to = \VanguardLTE\Transaction::whereIn('from_userId', $this->child_users_array)
                        ->orWhereIn('to_userId', $this->child_users_array)
                ...
                return Datatables::of($users_from_to)
                        ->make(true);
            }
            //get total users numbers per role
            $users_count = $this->users_count; 
            $shops_count = $this->shops_count; 
            $operators_count = $this->operators_count;
            return view('backend.dashboard.index', compact('users_count', 'shops_count', 'operators_count'));
        }
